...
struct CsrMatrix {
	...
	// starts
	//
	// if there are no non zero elements in row_to_val[i], then row_to_val[i+1] == row_to_val[i]
	//
	// row_to_val can point to nnz, which is a fictitious element at the end
        int *row_to_val;

	CsrMatrix(size_t nnz, size_t n, size_t m, double *val, size_t *col, int *row_to_val) : nnz(nnz), n(n), m(m), val(val), col(col), row_to_val(row_to_val) {}
	...
	double* to_dense() const {
		...
		for (int row= 0; row< this->n; row++) {
			...
			// M has values in this row from val_start_pointer to val_end_pointer
			int val_end_pointer;
			if (row + 1 == this->n) {
				val_end_pointer = nnz;
			} else {
			...
			for (int val_pointer = val_start_pointer; val_pointer < val_end_pointer; val_pointer++) {
				int col = this->col[val_pointer];
				//std::cout << "row: " << row << "  col: " << col << "  val: " << this->val[val_pointer] << " val_p: " << val_pointer << "\n";
				out[row * this->m + col] = this->val[val_pointer];
			}
	...
	double* matmult(const double *x) const {
		...
		for (int row= 0; row< this->n; row++) {
			...
			// Checks for sentinel values, -1
			// if so, skips this row because there are no values in M for it
			if (val_start_pointer == -1) {
				out[row] = 0.0;
				continue;
			}
		...
		for (int row = n-1; row > -1; row -- ) {
			...
			if (val_pointer == -1) {
				if (row == n-1) {
					...
					csr_row_to_val[row] = csr_row_to_val[row+1];
				}
			}
		}
